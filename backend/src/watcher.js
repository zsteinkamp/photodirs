...
const scanDirectory = async (dirName) => {
  ...
  try {
    subdirs = (
      await fsp.readdir(path.join(C.ALBUMS_ROOT, dirName), {
        withFileTypes: true,
      })
    ).filter(
      (dirEnt) =>
        dirEnt.isDirectory() && !dirEnt.name.match(C.MAC_FORBIDDEN_FILES_REGEX)
    )

    // recurse into subdirs before continuing
    await batchUtils.promiseAllInBatches(
      subdirs,
      (dirEnt) => scanDirectory(path.join(dirName, dirEnt.name)),
      10
    )
    ...
    await batchUtils.promiseAllInBatches(
      dirFiles,
      (fName) => fileObj.getFileObj(dirName, fName),
      10
    )
