...
const albumObjUtils = (module.exports = {
  ...
  getExtendedAlbumObj: async (extAlbumObj) => {
    ...
    if (await fileUtils.fileExists(extAlbumFname)) {
      ...
      try {
        subdirs = (
          await fsp.readdir(path.join(C.ALBUMS_ROOT, extAlbumObj.path), {
            withFileTypes: true,
          })
        ).filter(
          (dirEnt) =>
            dirEnt.isDirectory() &&
            !dirEnt.name.match(C.MAC_FORBIDDEN_FILES_REGEX)
        )
      } catch (e) {
      ...
      subdirAlbumJson.push(
        path.join(C.CACHE_ROOT, 'albums', extAlbumObj.path, 'album.json')
      )
      ...
      if (
        !(await fileUtils.isFileOlderThanAny(extAlbumFname, subdirAlbumJson))
      ) {
        logger.debug('RETURN CACHE', extAlbumFname)
        return JSON.parse(
          await fsp.readFile(extAlbumFname, { encoding: 'utf8' })
        )
      }
    }
    const dirs = []
    ...
    extAlbumObj.breadcrumb = await albumObjUtils.getBreadcrumbForPath(
      extAlbumObj.path
    )

    // TODO: Pagination / caching this metadata
    const albumResult = await batchUtils.promiseAllInBatches(
      dirs,
      (dir) =>
        albumObjUtils.getAlbumObj(path.join('/', extAlbumObj.path, dir.name)),
      10
    )
    ...
    const fileResult = await batchUtils.promiseAllInBatches(
      files,
      (file) => fileObj.getFileObj(extAlbumObj.path, file.name),
      10
    )
    //
    fileResult.sort((a, b) => {
  ...
  getAlbumObj: async (dirName) => {
    ...
    if (await fileUtils.fileExists(stdAlbumFname)) {
      ...
      const fileObjFnames = supportedFilesBare.map((fName) =>
        cache.getFileObjMetadataFname(dirName, fName)
      )
      ...
      if (!(await fileUtils.isFileOlderThanAny(stdAlbumFname, fileObjFnames))) {
        ...
        return JSON.parse(
          await fsp.readFile(stdAlbumFname, { encoding: 'utf8' })
        )
      }
    }
    ...
    if (!albumObj.date) {
      ...
      if (!albumObj.date) {
        ...
        const exifArr = await batchUtils.promiseAllInBatches(
          supportedFiles,
          (fName) => exifUtils.getExifObjForFile(path.join(dirName, fName)),
          10
        )
    ...
    if (albumObj.thumbnail) {
      ...
      albumObj.thumbnail = path.join(
        C.PHOTO_URL_BASE,
        uriPath,
        encodeURIComponent(albumObj.thumbnail)
      )
    } else {
      const thumbFname = await albumObjUtils.getAlbumDefaultThumbnailFilename(
        dirName
      )
      if (thumbFname) {
        albumObj.thumbnail = path.join(
