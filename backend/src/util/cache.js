...
module.exports = {
  ...
  makeResizeCachePath: (filePath, height, width) => {
    ...
    return `${filePath}^${width}x${height}.${fileTypes.getOutputTypeForFile(filePath)}`;
  },
  ...
  cleanUpCacheFor: async (albumFilePath) => {
    const files = await (fileUtils.globPromise(path.join(C.CACHE_ROOT, 'albums', `${albumFilePath}*`)));
